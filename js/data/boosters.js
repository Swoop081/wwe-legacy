import { cardsForSet, collectionCards } from "./collection.js?v=0.14.08";
import { addOwnedCard, addUniversePoints, cardOwnershipCap, grantSuperstarUnlockPackage, totalOwnedCopies, underTierOwnershipCap } from "./profile.js?v=0.14.08";
import { duplicateUniversePointsFor } from "./store.js?v=0.14.08";
import { sets } from "./sets.js?v=0.14.08";
import { isPlayerReleasedSetId, playerReleasedCollectibleSetIds } from "./release.js?v=0.14.08";
import { CARD_TIERS, TIER_PULL_WEIGHTS, rollCardTier } from "./variants.js?v=0.14.08";

export const BOOSTER_SIZE = 5;
export const MAX_VERY_RARE_PULLS = 1;
export const MAX_NON_NORMAL_PRINTINGS = 2;
export const MAX_SAPPHIRE_OR_RUBY_PRINTINGS = 1;
...
function buildPack(profile, rng, setId, now = new Date(), options = {}) {
  ...
  for (let i = 0; i < BOOSTER_SIZE; i += 1) {
    ...
    const highPremium = pullTier === "sapphire" || pullTier === "ruby";
    ...
    if (pullTier !== "normal") {
      ...
      if (pullTier === "sapphire" || pullTier === "ruby") sapphireOrRubyPrintings += 1;
    }
