import { totalMomentum } from "./utils.js?v=1.1.111";
import { healthRatio, healthZone } from "./health.js?v=1.1.111";
...
export function counterEligibility(state,playerId,incoming,counter){
 ...
 // Posture restrictions govern proactive Move play. In a Counter window, legality is defined by the incoming Move's exposed Counter state/target (or a card-specific Counter permission), so a stale posture flag must not invalidate an otherwise legal reversal.
