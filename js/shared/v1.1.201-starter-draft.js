...
import { superstarArtwork } from "../data/artwork.js?v=1.1.132";
...
function choice(id){const s=star(id),src=superstarArtwork[id]??placeholder;return `<button type="button" class="starter-draft-choice" data-starter-draft-choice="${id}"><span class="starter-draft-photo"><img src="${src}" alt="${s?.name??id}" onerror="this.onerror=null;this.src='${placeholder}'"></span><strong>${s?.name??id}</strong><small>${s?.nickname??"CHOOSE SUPERSTAR"}</small></button>`;}
function render(){
...
function pick(id){const draft=ensureRoute(),round=draft[step];if(!round?.choices.includes(id))return;selections[step]=id;if(step<3){step++;render();return;}const profile=createStarterDraftProfile(draft,selections);saveProfile(profile);location.reload();}
