import { assetUrl, BUILD_VERSION } from "../config/build.js?v=1.1.354";
import { fetchLatestBuild, isNewerBuild, updateNavigationUrl } from "../config/update.js?v=1.1.354";
import { superstars } from "../data/superstars.js?v=1.1.354";
import { decks } from "../data/decks.js?v=1.1.354";
import { sets } from "../data/sets.js?v=1.1.354";
import { playerReleasedCollectibleSetIds, isPlayerReleasedSetId, isPlayerVisibleSuperstar } from "../data/release.js?v=1.1.354";
import { collectionCards, setCollection, setCollections, cardsForSet } from "../data/collection.js?v=1.1.354";
import { artworkFor, superstarArtwork, menuSuperstarPhotoFor, finalBossRockMenuArtwork, superstarCardArtFor, superstarHeadshotFor, finishedCardArtFor, legacyFinishedCardArtFor, layeredCardArtFor } from "../data/artwork.js?v=1.1.354";
import { isAnimatedCardEligible, canonicalAnimatedCardPaths } from "../data/animated-card-art.js?v=1.1.354";
import { STARTER_CHOICES, PREMIERE_STARTER_MALES, PREMIERE_STARTER_FEMALES, createProfile, hasSuperstar, loadProfile, saveProfile, resetProfile, profilePersistenceStatus, setDeckAssistance, ownedCount } from "../data/profile.js?v=1.1.354";
import { openBooster, grantBooster, grantRandomBoosters, boosterCreditsFor, finalizePackUniversePoints } from "../data/boosters.js?v=1.1.354";
import { STORE_BOOSTER_PRICE, STORE_SUPERSTAR_PRICE, storeRotation, storeSuperstars, storeLeadOffCards, purchaseStoreBooster, purchaseStoreSuperstar } from "../data/store.js?v=1.1.354";
import { randomExhibitionOpponent } from "../data/matchmaking.js?v=1.1.354";
import { buildPlayableDeck, findPackUpgrades, applyUpgrade } from "../data/deck-assistant.js?v=1.1.354";
import { applyCardTier, CARD_TIERS, highestOwnedTier, normalizeCardTier, tierLabel, tierRank } from "../data/variants.js?v=1.1.354";
import { scaleCpuDeckToPlayer } from "../data/cpu-tier-scaling.js?v=1.1.354";
import { MatchEngine } from "../engine/MatchEngine.js?v=1.1.354";
import { canPlayMomentum, canPlayEntrance, canPlayAction, canPlayManager, canPlaySpecial, effectiveTotalMomentum, moveEligibility, canCounter, counterEligibility, autoCounterEligibility, autoCounterCost, canAttemptPin, canPlayPinEscape, submissionThreshold, canReturnToRing, canFollowOutside } from "../engine/rules.js?v=1.1.354";
import { totalMomentum } from "../engine/utils.js?v=1.1.354";
import { healthZone } from "../engine/health.js?v=1.1.354";
import { decisionOwner } from "../ai/WrestlingAI.js?v=1.1.354";
import { advanceCpuUntilHuman } from "./turn-driver.js?v=1.1.354";
import { reconstructCurrentPlayPile } from "./play-pile.js?v=1.1.354";
import { playPileMatStyle } from "./play-pile-mats.js?v=1.1.354";
import { LADDER_LIVES, LADDER_LENGTH, ladderState, startLadderRun, currentLadderOpponent, recordLadderMatch } from "../data/ladder.js?v=1.1.354";
import { KING_OF_THE_RING_ROUNDS, kingOfTheRingState, startKingOfTheRing, currentKingOfTheRingOpponent, recordKingOfTheRingMatch, markKingOfTheRingCoronationSeen, resetKingOfTheRing } from "../data/king-of-the-ring.js?v=1.1.354";
import { CHAMPIONSHIP_ROAD_LENGTH, CHAMPIONSHIP_STAGES, CHAMPIONSHIP_DIFFICULTY_ORDER, CHAMPIONSHIP_DIFFICULTIES, CHAMPIONSHIP_ROAD_SECTIONS, CHAMPIONSHIP_ROAD_OPPONENTS, championshipRoadState, championshipRoadForSuperstar, selectChampionshipRoadSuperstar, championshipDifficultyUnlocked, championshipRoadDifficultyModifier, championshipRoadSectionForStage, championshipRoadOpponentsForSuperstar, startChampionshipRoad, currentChampionshipOpponent, recordChampionshipMatch, resetChampionshipRoad } from "../data/championship-road.js?v=1.1.354";
import { LIVE_EVENT_LENGTH, LIVE_EVENT_WIN_UP, LIVE_EVENT_CLEAR_BOOSTERS, DAILY_LIVE_EVENT_SET_XP, activeLiveEventTowers, liveEventTowerByKey, liveEventTowerState, startLiveEventTower, changeLiveEventTowerSuperstar, currentLiveEventTowerOpponent, currentLiveEventTowerStage, recordLiveEventTowerMatch, liveEventRotation, liveEventStage, weeklyLiveEventState, dailyLiveEventSetStatus } from "../data/live-events.js?v=1.1.354";
import { challengeState, claimChallenge, recordCompletedMatchChallenges } from "../data/challenges.js?v=1.1.354";
import { CAREER_MODES, careerRecord, achievementProgress, recordCareerMatch, refreshCareerAchievements } from "../data/career.js?v=1.1.354";
import { COLLECTION_MILESTONES, EMERALD_MILESTONES, SAPPHIRE_MILESTONES, RUBY_MILESTONES, setProgressState, collectionProgress, availableMilestoneRewards, claimMilestone } from "../data/set-progression.js?v=1.1.354";
import { MOVE_TYPE_LABELS } from "../data/move-types.js?v=1.1.354";
import { COUNTER_STATE_LABELS, SUBMISSION_TARGET_LABELS } from "../data/counter-states.js?v=1.1.354";
import { CATALOGUE_PAGE_SIZE, defaultCatalogueFilters, catalogueOptions, filterAndSortCatalogue, superstarIdsForCard, isSharedCard } from "../data/catalogue.js?v=1.1.354";
import { DECK_LAB_CATEGORIES, createDeckDraft, recommendedDeckDraft, optimizeDeck, aggregateDeck, eligibleOwnedCards, allOwnedEntrances, ownedCardsForCategory, addCardToDraft, removeCardFromDraft, replaceLeadOffSlot, validateDeckDraft, materializeDraft, leadOffIds, buildOwnedRecommendedDraft, buildBestOwnedRecommendedDraft, recommendedDeckComparison, recommendedEntranceId, recommendedDeckMissingCount, autoFillOwnedDraft, recommendedCategoryCounts, currentCategoryCounts, cardEligibilityForSuperstar, entranceEligibilityForSuperstar, selectedEntranceId, setSelectedEntrance, ownedTotal, categoryForCard } from "../data/deck-builder.js?v=1.1.354";
import { RECOMMENDED_DECK_SHAPE } from "../data/deck-health.js?v=1.1.354";
import { SEASON_1, SEASON_TIER_COUNT, XP_PER_TIER, MATCH_XP, seasonState, seasonTier, seasonLevelProgress, seasonTimeRemaining, awardMatchSeasonXp, tierReward, claimSeasonTier, claimAllSeasonTiers, freePackStatus, claimFreeSeasonBooster } from "../data/seasons.js?v=1.1.354";
import { GAME_RULE_SECTIONS, PIN_CHANCE_TABLE, LIVE_EVENT_WEEK } from "../data/game-rules.js?v=1.1.354";
import { SAVE_FILENAME, exportSaveToFiles, readSaveFile, saveImportRollback, loadImportRollback, clearImportRollback, backupMetadata } from "../data/save-backup.js?v=1.1.354";
import { DAILY_SPIN_WEDGES, dailySpinState, spinDaily } from "../data/daily-spin.js?v=1.1.354";
import { MERCH_ITEMS, MERCH_BY_ID, activeMerchItem, merchEligibilityForSuperstar, equipMerch, discardActiveMerch, merchMatchModifier, consumeActiveMerchMatch } from "../data/merch.js?v=1.1.354";
import { SUPERSTAR_VARIANTS, SUPERSTAR_VARIANT_BY_ID, equippedSuperstarVariant, equipSuperstarVariant, superstarVariantMatchModifier } from "../data/superstar-variants.js?v=1.1.354";
import { canEnterSurvivorSeries, survivorSeriesState, currentSurvivorSeriesRun, startSurvivorSeries, setSurvivorChallenge, autoSurvivorChallenge, resolveSurvivorSeriesMatch, resetSurvivorSeries } from "../data/survivor-series-mode.js?v=1.1.354";
...
// Runtime identity lookup must include every active authored Superstar. Release filtering belongs to catalogue/store discovery, not to saved starter decks or match setup.
const superstarById = Object.fromEntries(Object.values(superstars).filter(star => !star.developmentOnly).map(star => [star.id, star]));
...
function setChrome({ hideTopbar = false } = {}) {
  ...
  if (bar) {
    ...
    if (showBar) {
      ...
      // Chrome must never block screen navigation if an optional notification subsystem has stale save data.
      let chromeNotices = { boosters: 0, challenges: 0, seasons: 0 };
      try { chromeNotices = attentionState(); } catch (error) { console.warn("Chrome attention state unavailable", error); }
      bar.classList.toggle("has-season-reward", (chromeNotices.seasons ?? 0) > 0);
  ...
  if (mobileNav) {
    ...
    let notices = { boosters: 0, challenges: 0, seasons: 0 };
    try { notices = attentionState(); } catch (error) { console.warn("Mobile nav attention state unavailable", error); }
    mobileNav.querySelectorAll("[data-mobile-nav]").forEach(button => {
...
function showProfile() {
  if (!profile) { screen = "starter"; renderStarter(); return; }
  screen = "profile"; message = "";
  try {
    renderProfile();
  } catch (error) {
    console.error("My Legacy render failed", error);
    setChrome();
    const root = $("#game");
    if (root) root.innerHTML = `<section class="profile-screen premium-screen profile-premium profile-command-screen"><header class="premium-hub-heading hub-legacy"><div><span>YOUR CAREER</span><h1>MY <em>LEGACY</em></h1><p>Records · Rulebook · save & backup</p></div></header><section class="premium-panel"><div class="section-title"><div><h3>My Legacy</h3><small>The career screen hit a data error in this save.</small></div></div><p>${String(error?.message ?? error ?? "Unknown render error")}</p><button id="legacy-retry" class="start-match">RETRY</button></section></section>`;
    $("#legacy-retry")?.addEventListener("click", showProfile);
  }
}
function showRules() {
  if (!profile) { screen = "starter"; renderStarter(); return; }
...
function showLiveEvents() {
  if (!profile) { screen = "starter"; renderStarter(); return; }
  selectedLiveEventKey = null; liveEventSuperstarSwapTowerKey = null; liveEventMatchupOpen = false;
  screen = "live-events"; message = ""; setChrome();
  try {
    renderLiveEvents();
  } catch (error) {
    // Premiere relaunch can leave legacy Live Event progress referring to removed
    // event/set data. Never allow that stale optional state to make the route inert.
    console.error("Live Events route recovery", error);
    const root = $("#game");
    if (root) root.innerHTML = `<section class="live-events-hub premium-screen"><header class="live-events-hub-heading">${modeLogoMarkup("live-event", false)}<p>Live Events are available. Old pre-relaunch event progress could not be loaded.</p></header><section class="premium-panel"><h3>LIVE EVENTS RESET REQUIRED</h3><p>Your Collection and Superstar decks are unchanged. Reset only the stale Live Event run data, then reopen this mode.</p><button id="live-events-reset-retry" class="start-match">RESET LIVE EVENTS & RETRY</button><button id="live-events-recovery-back" class="nav-button">BACK TO PLAY</button></section></section>`;
    $("#live-events-reset-retry")?.addEventListener("click", () => {
      // New Premiere definitions are now authoritative; retry without mutating the
      // player's Collection/decks. Legacy event keys are ignored by the data layer.
      selectedLiveEventKey = null; message = ""; renderLiveEvents();
    });
    $("#live-events-recovery-back")?.addEventListener("click", showPlayMenu);
  }
}
...
function showDeckBuilder(starId = null) {
  if (!profile) { screen = "starter"; renderStarter(); return; }
  // Deck Lab is a core route. Enter the screen first, then let its renderer
  // recover any stale/partial roster state instead of allowing a nav tap to
  // disappear when old save metadata is inconsistent.
  screen = "deck-builder";
  message = "";
  const unlocked = [...new Set((profile.unlockedSuperstars ?? []).filter(id => superstarById[id]))];
  if (!unlocked.length) {
    const starters = (profile.starterIds ?? [profile.starterId]).filter(id => superstarById[id]);
    profile.unlockedSuperstars = [...new Set(starters)];
    saveProfile(profile);
    unlocked.push(...profile.unlockedSuperstars);
  }
  ...
  try {
    ...
    console.error("Deck Lab route recovery", error);
    const root = $("#game");
    if (root) root.innerHTML = `<section class="deck-builder-screen deck-lab-screen premium-screen"><header class="deck-lab-editor-head"><button id="deck-lab-recovery-back" type="button" class="ghost">← Home</button><div><span>DECK LAB</span><h2>Deck Lab Recovery</h2><p>The deck screen hit a data error. Your saved cards and decks are unchanged.</p></div></header><p class="setup-message">${String(error?.message ?? error ?? "Unknown Deck Lab error")}</p><button id="deck-lab-retry" class="start-match">RETRY DECK LAB</button></section>`;
    $("#deck-lab-recovery-back")?.addEventListener("click", showMainMenu);
    $("#deck-lab-retry")?.addEventListener("click", () => showDeckBuilder(starId));
  }
}
...
function maybeCelebrateFocusedSuperstarPull(delay = 260) {
  ...
  saveProfile(profile);
  setTimeout(() => {
    if (packStage !== "reveal" || lastPack?.[boosterFocusIndex] !== pull) return;
...
function renderBoostersUnsafe() {
  ...
  const packSubtitle="SERIES 1 · 5 CARDS · AMETHYST CHASE";
  ...
  if (packStage === "sealed") {
    ...
    const converted=Boolean(p.universePointsValue) && (convertedPackCards.has(boosterFocusIndex) || p.universePointsCredited);
...
function renderSeasons() {
  ...
  const tierRoad = Array.from({length: SEASON_TIER_COUNT}, (_, i) => i + 1).map(tier => {
    ...
    const rewardSub = isFinalBossCard ? `${reward.label}` : reward.kind === "universe-points" ? `UNIVERSE POINTS` : setName;
  ...
  root.innerHTML = `<section class="seasons-screen premium-screen season-road-redesign">
    <section class="season-anchor-shell">
      ...
      <section class="season-road-command"><div class="season-current-tier"><small>CURRENT TIER</small><span>${progress.xp.toLocaleString()} / ${(SEASON_TIER_COUNT*XP_PER_TIER).toLocaleString()} XP</span></div><div class="season-rewards-ready"><small>REWARDS READY</small><strong>${claimable.length}</strong></div><div class="season-universe-points"><small>UNIVERSE POINTS</small><strong>${(profile.universePoints??0).toLocaleString()} UP</strong></div></section>
      ${message ? `<p class="setup-message">${message}</p>` : ''}
...
function renderChallenges() {
  ...
  const milestoneSetMarkup = ({set,progress,state,rewards}) => {
  ...
  const premiereMilestone = setRows.find(row => row.set.id === "premiere");
  const rewardMilestones = setRows.filter(row => row.set.id !== "premiere").sort((a,b)=>String(a.set.displayName).localeCompare(String(b.set.displayName)));
  const rewardReadyCount = rewardMilestones.reduce((n,row)=>n+row.rewards.collection.length+row.rewards.emerald.length+row.rewards.sapphire.length+row.rewards.ruby.length,0);
  const milestoneSections = `${premiereMilestone ? milestoneSetMarkup(premiereMilestone) : ''}
    <details class="challenge-reward-sets challenge-collapsible">
      <summary class="set-milestone-heading challenge-reward-sets-heading"><div class="set-milestone-heading-copy"><span>SET MILESTONES</span><h3>REWARDS</h3><small>Chase and reward sets</small></div><b>${rewardReadyCount ? `${rewardReadyCount} READY` : 'VIEW SETS'} <i aria-hidden="true">⌄</i></b></summary>
      <div class="challenge-reward-set-list">${rewardMilestones.map(milestoneSetMarkup).join('')}</div>
    </details>`;
...
function startCurrentChampionshipMatch() {
  ...
  startMatch(run.superstarId, opponentId, {
    ...
    eventMeta: { rewardSetId: roadSection?.setId ?? "premiere", championshipSectionId: roadSection?.id ?? null }
  });
}
function renderChampionship(){
  ...
  const roadSwitcher = unlocked.map(star=>{
    ...
    return `<button type="button" class="champ-superstar-road ${star.id===chosenId?'active':''}" data-champ-superstar="${star.id}"><span class="champ-road-player-card">${superstarPreviewCardMarkup(star.id,"champ-road-switcher-superstar-card")}</span><strong>${star.name}</strong><span>${progress}/${CHAMPIONSHIP_ROAD_LENGTH}</span><small>${status}</small></button>`;
  }).join('');
...
function modeLogoMarkup(mode, compact = false) {
  const modes = {
    ...
    championship: { kicker: "16 MATCHES · FOUR DIFFICULTIES", top: "CHAMPIONSHIP", bottom: "ROAD" },
...
function selectionCarouselMarkup(stars, selectedId, context, labelFor = null) {
  const liveRouteStyle = context === "live-event-select" || context === "live-event-swap" || context === "ladder-select";
  if (liveRouteStyle) {
...
function renderMainMenu() {
  // A menu route can survive a reset/recovery while the profile itself is null.
  // Never dereference it during boot; return to onboarding instead.
  if (!profile) { screen = "starter"; renderStarter(); return; }
  setChrome();
  const root = $("#game");
  const starter = superstarById[profile.starterId] ?? superstars[profile.starterId] ?? roster[0];
  if (!starter) { screen = "starter"; message = ""; renderStarter(); return; }
  ...
  const collectiblePrintingCount = card => card.fixedPrintingTier ? 1 : CARD_TIERS.length;
  const ownedPrintingCount = card => card.fixedPrintingTier
    ? (ownedCount(profile,card.id,card.fixedPrintingTier) > 0 ? 1 : 0)
    : CARD_TIERS.filter(tier => ownedCount(profile,card.id,tier) > 0).length;
  const ownedUnique = visibleCards.reduce((sum,card)=>sum+ownedPrintingCount(card),0);
  const totalCollectiblePrintings = visibleCards.reduce((sum,card)=>sum+collectiblePrintingCount(card),0);
  const ownedCopies = visibleCards.reduce((sum, card) => sum + CARD_TIERS.reduce((sum,tier)=>sum+ownedCount(profile,card.id,tier),0), 0);
  const premiereRosterIds = [...PREMIERE_STARTER_MALES, ...PREMIERE_STARTER_FEMALES, "la-knight"];
  const launchUnlocked = (profile.unlockedSuperstars ?? []).filter(id => premiereRosterIds.includes(id)).length;
  ...
  root.innerHTML = `<section class="main-menu-screen premium-screen legacy-home-v3">
    <section class="legacy-home-stage" aria-label="WWE Legacy home">
      ...
      <div class="legacy-stage-stats">
        <span><small>SUPERSTARS</small><b>${launchUnlocked}/${premiereRosterIds.length}</b></span>
        <span><small>COLLECTION</small><b>${ownedUnique}/${totalCollectiblePrintings}</b></span>
  ...
  $("#menu-profile")?.addEventListener("pointerup", event => { if (event.pointerType === "touch") showProfile(); });
...
function renderPlayMenu() {
  ...
  const root = $("#game");
  const now = new Date();
  // Relaunch-safe mode hub: a stale pre-Premiere mode save must never make PLAY inert.
  let towers = [], dailyTower = null, dailyState = null;
  try {
    towers = activeLiveEventTowers(now, profile) ?? [];
    dailyTower = towers.find(tower => tower.cadence === "daily") ?? towers[0] ?? null;
    dailyState = dailyTower ? liveEventTowerState(profile, dailyTower.key, now)?.state : null;
  } catch (error) { console.warn("Live Events state reset required", error); }
  ...
  let kotrHub = {};
  try { kotrHub = kingOfTheRingState(profile) ?? {}; } catch (error) { console.warn("King of the Ring state reset required", error); }
  ...
  let mitb = { activeRun:null, dailyCleared:false };
  try { mitb = ladderState(profile, now) ?? mitb; } catch (error) { console.warn("Money in the Bank state reset required", error); }
...
function renderStarterSummary(starterIds = profile?.starterIds ?? []) {
  ...
  const cards = ids.map(id => {
    const index = PREMIERE_STARTER_MALES.includes(id) ? PREMIERE_STARTER_MALES.indexOf(id)+1 : PREMIERE_STARTER_FEMALES.indexOf(id)+9;
    const cardId = `PREM${String(index).padStart(2,"0")}`;
...
function renderStarter() {
  ...
  const starterPulls = [
    { gender: "male", id: maleId, cardId: `PREM${String(PREMIERE_STARTER_MALES.indexOf(maleId) + 1).padStart(2,"0")}` },
    { gender: "female", id: femaleId, cardId: `PREM${String(PREMIERE_STARTER_FEMALES.indexOf(femaleId) + 9).padStart(2,"0")}` }
  ];
  let starterPackIndex = 0;
  let starterPackStage = "sealed";

  const starterCard = pull => collectionById.get(pull.cardId) ?? collectionCards.find(card => card.cardCode === pull.cardId || card.id === pull.cardId) ?? null;
  const packMarkup = () => `<span class="booster-pack physical-booster-pack pack-set-premiere sealed-pack starter-premiere-physical-pack${starterPackStage === "opening" ? " is-opening" : ""}"><span class="pack-crimp pack-crimp-top" aria-hidden="true"></span><span class="pack-side-seam pack-side-seam-left" aria-hidden="true"></span><span class="pack-side-seam pack-side-seam-right" aria-hidden="true"></span><span class="pack-wrapper-face"><img class="starter-premiere-logo-direct" src="${assetUrl("assets/images/premiere-logo.png")}" alt="Premiere"><b class="pack-series-name">${starterPackIndex === 0 ? "MALE SUPERSTAR" : "FEMALE SUPERSTAR"}</b><small class="pack-contents-line">1 SUPERSTAR CARD</small></span><span class="pack-crimp pack-crimp-bottom" aria-hidden="true"></span><span class="pack-tear-notch" aria-hidden="true"></span><i class="pack-foil-sheen" aria-hidden="true"></i>${starterPackStage === "opening" ? '<span class="pack-tear" aria-hidden="true"></span>' : ""}</span>`;

  const paint = () => {
    const pull = starterPulls[starterPackIndex];
    const card = starterCard(pull);
    const ordinal = starterPackIndex + 1;
    let stage = "";
    if (starterPackStage === "sealed") {
      stage = `<section class="pack-opening-stage sealed-pack-stage starter-single-pack-stage">
        <button type="button" id="starter-rip-pack" class="sealed-pack-button" aria-label="Open Premiere starter pack ${ordinal}">
          ${packMarkup()}<strong>TAP TO RIP</strong><small>PACK ${ordinal} OF 2 · ${pull.gender.toUpperCase()} SUPERSTAR</small>
        </button>
      </section>`;
    } else if (starterPackStage === "opening") {
      stage = `<section class="pack-opening-stage ripping-pack-stage starter-single-pack-stage"><div class="pack-rip-flash"></div>${packMarkup()}<strong class="pack-rip-callout">RIP!</strong></section>`;
    } else {
      stage = `<section class="starter-pack-reveal-rebuilt" role="button" tabindex="0" aria-label="${starterPackIndex === 0 ? "Continue to female Superstar pack" : "Continue with these starter Superstars"}">
        <h1>${pull.gender.toUpperCase()} SUPERSTAR</h1>
        <div class="starter-pack-reveal-card-host">
          ${card ? collectibleCardMarkup(card,{flipped:false,tier:"normal",interactive:false,eagerArt:true,extraClass:"starter-pack-reveal-card"}) : `<div class="starter-card-error">${pull.cardId}</div>`}
        </div>
        <button type="button" class="start-match starter-reveal-continue">${starterPackIndex === 0 ? "OPEN FEMALE PACK" : "CONTINUE"}</button>
      </section>`;
    }
    root.innerHTML = `<section class="starter-screen premiere-starter-onboarding starter-pack-opening-flow">
      <div class="starter-hero"><span class="eyebrow">WELCOME TO WWE LEGACY</span><h2>OPEN YOUR PREMIERE PACKS</h2><p>Open your male Superstar pack, then your female Superstar pack.</p></div>
      ${stage}
    </section>`;
    globalThis.WWELegacyRenderCardFaces?.(root);

    $("#starter-rip-pack")?.addEventListener("click", () => {
      starterPackStage = "opening";
      paint();
      setTimeout(() => { starterPackStage = "reveal"; paint(); }, 420);
    });
    const advanceStarterReveal = () => {
      if (starterPackStage !== "reveal") return;
      if (starterPackIndex === 0) {
        starterPackIndex = 1;
        starterPackStage = "sealed";
        paint();
      } else {
        try { chooseStarter([maleId,femaleId]); }
        catch (error) {
    ...
    // The shared card renderer can repaint the card's internal DOM after this
    // screen is painted. Bind progression to the stable reveal stage instead of
    // an element inside the rendered card so onboarding can never dead-end.
    const revealStage = root.querySelector(".starter-pack-reveal-rebuilt");
    revealStage?.addEventListener("click", advanceStarterReveal);
    revealStage?.addEventListener("keydown", event => {
      if (event.key === "Enter" || event.key === " ") {
        event.preventDefault();
        advanceStarterReveal();
      }
    });
  };
  paint();
...
function collectibleCardMarkup(card, { flipped = false, tier = null, foil = null, extraClass = "", footer = "", flipAttr = "", interactive = true, eagerArt = false, preferFinished = false, finishedAnimationShell = false } = {}) {
  ...
  const premiereFrontLogo = card.setId === "premiere" ? `<img class="ccg-premiere-front-set-logo" src="${assetUrl("assets/images/premiere-logo.png")}" alt="" aria-hidden="true">` : "";
  ...
  const frontMarkup = forcedFinishedFront
    ...
    : layeredFront
      ? `<span class="ccg-card-art ${superstarFront ? "ccg-superstar-full-art" : moveFront ? "ccg-move-full-art" : ""}">${superstarFront && card.setId === "premiere" ? `<img class="ccg-premiere-card-background" src="${assetUrl("assets/images/premiere-background.jpg")}" alt="" aria-hidden="true">` : ""}${cardArtFace(card,{eager:eagerArt})}</span>${superstarFront ? `${superstarNameplateMarkup(card)}${layeredSuperstarRarityMarkup(card)}` : layeredFrontOverlayMarkup(card)}${layeredCardStudioInkMarkup(card)}`
    : superstarFront
  ...
  return `<${rootTag} ${rootAttrs} data-card-tier="${resolvedTier}" class="ccg-card ${displayFlipped ? "is-flipped" : ""} ${missingCustomFront ? "uses-rules-fallback force-rules-face" : ""} ${setClass} ${typeClass} ${finisherClass} rarity-${card.rarity ?? 1} ${card.ultraRare ? "is-ultra-rare" : ""} ${tierClass} ${resolvedTier !== "normal" ? "is-tier-glow" : ""} ${superstarFront ? "is-full-art-superstar" : ""} ${finishedFront || layeredFront ? "is-full-art-finished" : ""} ${layeredFront ? "is-layered-front" : ""} ${tribalChiefLiveFront ? "has-tribal-chief-live-front" : ""} ${isPremiumRewardCard(card) ? "is-premium-reward-card" : ""} ${moveFront && (finishedFront || layeredFront) ? "is-full-art-move" : ""} ${extraClass}">
    <span class="ccg-card-inner">
      <span class="ccg-card-face ccg-card-front">${frontMarkup}${animatedSurface}${premiereFrontLogo}${resolvedTier !== "normal" ? `<span class="ccg-tier-overlay" aria-hidden="true"></span>` : ""}<span class="ccg-print-frame" aria-hidden="true"></span></span>
...
function renderCollection() {
  ...
  root.innerHTML = `<section class="collection-screen premium-screen ${themeClass} collection-density-screen collection-compact-redesign">
    ...
    <section class="catalogue-grid collectible-catalogue">${displayed.length ? displayed.map(card => {
      ...
      const printingTiers = card.fixedPrintingTier ? [card.fixedPrintingTier] : CARD_TIERS;
      return `<article class="catalogue-collectible ${card.kind === 'superstar' && !hasSuperstar(profile, card.superstarId) ? 'collection-locked' : ''}"><div class="catalogue-printing-row">${printingTiers.map(tier=>`<div class="catalogue-printing"><div class="catalogue-printing-card">${collectibleCardMarkup(card,{flipped:flippedCollectionCards.has(`${card.id}:${tier}`),tier,flipAttr:`data-flip-collection="${card.id}:${tier}"`})}</div><b>${tierLabel(tier)}</b><small>${ownedCount(profile,card.id,tier)} OWNED</small></div>`).join('')}</div><div class="catalogue-under-card"><span>${card.cardCode}</span><b>${card.name}</b><small>${cardSet.rarityLabels[card.rarity]}</small>${card.kind==='superstar'&&hasSuperstar(profile,card.superstarId)?`<button type="button" class="favourite-star-button ${(profile.favouriteSuperstars??[]).includes(card.superstarId)?'active':''}" data-favourite-star="${card.superstarId}">${(profile.favouriteSuperstars??[]).includes(card.superstarId)?'★ FAVOURITE':'☆ ADD FAVOURITE'}</button>`:''}</div></article>`;
...
function renderDeckBuilder() {
  ...
  // Do not run Deck Lab's owned roster back through release-set filtering.
  // Relaunch Premiere starters reuse historical Superstar records, so the
  // profile's explicit unlocked roster is authoritative here.
  const fav = new Set(profile?.favouriteSuperstars ?? []);
  const unlocked = [...new Set(profile.unlockedSuperstars)]
    .map(id => superstarById[id])
    .filter(Boolean)
  ...
  const superstarCard = superstarCollectibleFor(star.id);
  ...
  root.innerHTML = `<section class="deck-builder-screen deck-lab-screen premium-screen">
    ...
    <section class="deck-merch-loadout ${activeMerchForStar ? 'has-active-merch' : 'is-empty'}">
...
const activateMobileNav = button => {
...
document.querySelectorAll("[data-mobile-nav]").forEach(button => {
  // Use the native click activation only. iOS synthesizes click after touch/pointer
  // activation; handling pointerup as well allowed a Deck Lab tap to navigate twice
  // and the second hit could land on Play after the rail re-rendered.
  button.addEventListener("click", event => {
    event.preventDefault();
    activateMobileNav(button);
  });
});
