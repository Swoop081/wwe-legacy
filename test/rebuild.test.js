...
import { superstars } from "../js/data/superstars.js?v=1.1.114";
import { decks } from "../js/data/decks.js?v=1.1.114";
import { allGameplayCards } from "../js/data/content.js?v=1.1.114";
import { collectionCards } from "../js/data/collection.js?v=1.1.114";
import { CARD_NUMBER_BY_ID } from "../js/data/card-number-manifest.js?v=1.1.114";
import { createProfile, migrateProfile, unlockSuperstar, addOwnedCard, addUniversePoints, totalOwnedCopies, cardOwnershipCap, hasSuperstar } from "../js/data/profile.js?v=1.1.114";
import { grantBooster, openBooster, finalizePackUniversePoints, boosterEligible } from "../js/data/boosters.js?v=1.1.114";
import { STORE_BOOSTER_PRICE, STORE_SUPERSTAR_PRICE, storeRotation, storeLeadOffCards, purchaseStoreBooster, purchaseStoreSuperstar } from "../js/data/store.js?v=1.1.114";
import { exhibitionOpponentIds, randomExhibitionOpponent } from "../js/data/matchmaking.js?v=1.1.114";
import { buildOwnedRecommendedDraft, autoFillOwnedDraft, recommendedDeckDraft, cardEligibilityForSuperstar, replaceLeadOffSlot, validateDeckDraft, selectedEntranceId, setSelectedEntrance, recommendedCategoryCounts, currentCategoryCounts } from "../js/data/deck-builder.js?v=1.1.114";
import { tierReward, claimSeasonTier, SEASON_1, SEASON_2_COMPLETION_SUPERSTAR } from "../js/data/seasons.js?v=1.1.114";
import { seasonExclusiveSuperstars } from "../js/data/season-exclusive.js?v=1.1.114";
import { season2GoldbergCards } from "../js/data/season2-goldberg-cards.js?v=1.1.114";
import { MatchEngine } from "../js/engine/MatchEngine.js?v=1.1.114";
import { moveEligibility, canPlayMomentum, canAttemptPin, canPlayAction } from "../js/engine/rules.js?v=1.1.114";
import { decisionOwner, cpuDecision, executeCpuDecision } from "../js/ai/WrestlingAI.js?v=1.1.114";
import { healthZone } from "../js/engine/health.js?v=1.1.114";
import { LAUNCH_LIVE_SET_IDS, isLaunchLiveSetId, isPlayerReleasedSetId, isUnreleasedSetId } from "../js/data/release.js?v=1.1.114";
...
test("canonical collector manifest is gap-free and matches Collection plus Card Art Studio for every active card", async()=>{
  ...
  const { CARD_NUMBER_MANIFEST, CARD_NUMBER_BY_ID } = await import("../js/data/card-number-manifest.js?v=1.1.114");
...
test("Penta Worlds Collide package is locked and playable",()=>{
  ...
  assert.equal(q.players.p1.momentum.agility,1); assert.equal(q.players.p1.momentum.strength,1); assert.equal(q.players.p1.adrenaline,0);
...
test("v0.11.99 Play Pile inspector uses a mobile-safe non-nested hit target and HUD headshots fill their viewport", async()=>{
  ...
  const ui=fs.readFileSync(new URL('../js/ui/app.js',import.meta.url),'utf8');
  const css=fs.readFileSync(new URL('../css/game.css',import.meta.url),'utf8');
  assert.ok(ui.includes('class="play-pile-card-trigger" data-open-play-pile-index="${index}" role="button" tabindex="0"'));
...
test("v0.12.08 Season and Challenges use icon-led premium dashboard components", async()=>{
  ...
  assert.ok(ui.includes('season-road-reward-visual'));
