...
import { counterEligibility } from '../js/engine/rules.js?v=1.1.114';
...
test('v0.12.35 Arm Drag counter-attacks are terminal',()=>{
  const arm=byId('arm-drag'), ddt=byName('DDT');
  assert.ok(arm&&ddt);
  ...
  assert.notEqual(s.phase,'COUNTER','offensive Arm Drag must resolve immediately');
...
test('v0.12.35 rules reject every non Punch-Elbow reply to an offensive counter-attack',()=>{
  const arm=byId('arm-drag'), jaw=byId('jawbreaker'), punch=byId('punch'), elbow=byId('elbow');
