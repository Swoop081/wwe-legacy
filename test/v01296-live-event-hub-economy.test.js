...
import { activeLiveEventTowers, liveEventTowerState, startLiveEventTower, currentLiveEventTowerOpponent, recordLiveEventTowerMatch, LIVE_EVENT_LENGTH } from '../js/data/live-events.js?v=1.1.114';
import { createProfile } from '../js/data/profile.js?v=1.1.114';
import { STORE_SUPERSTAR_PRICE } from '../js/data/store.js?v=1.1.114';
import { superstars } from '../js/data/superstars.js?v=1.1.114';
import { isLaunchLiveSetId } from '../js/data/release.js?v=1.1.114';
...
test('v1.1.20 retires Birthday Bash from the three daily Live Event slots',()=>{
  for (const date of ['2027-07-12T10:00:00','2027-07-13T10:00:00']) {
    const towers=activeLiveEventTowers(new Date(date));
    assert.equal(towers.length,3);
    assert.equal(towers.some(t=>t.cadence==='birthday'||/birthday-bash/.test(t.event.id)),false);
  }
});
