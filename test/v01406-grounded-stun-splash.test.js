...
import { counterEligibility, autoCounterEligibility } from '../js/engine/rules.js?v=0.14.06';
...
test('v0.14.06 grounded Superstar stands automatically when gaining Control',()=>{
...
test('v0.14.06 Stun blocks both normal Counter and Auto Counter',()=>{
...
test('v0.14.06 existing Stun clears after one successful opponent Move',()=>{
  const g=make(),s=g.state(),d=s.players.p2;
...
test('v0.14.06 a fresh Stun inflicted by the same connecting Move survives',()=>{
  const g=make(),s=g.state(),d=s.players.p2;
  d.status.stunnedTurns=1; d.stun=1;
...
test('v0.14.06 Stun clears when the stunned controller commits a legal Move',()=>{
...
test('v0.14.06 Stun clears when the stunned controller passes',()=>{
...
test('v0.14.06 launch splash is a 50-tier Cena reward splash with no obsolete 100-tier copy',()=>{
