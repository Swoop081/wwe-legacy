...
test('v0.14.20 locks the roster audit numerical changes',()=>{
  ...
  assert.equal(star('doink-the-clown').hp,62);
  ...
  assert.equal(card('doink-stump-puller').submission.pressure,6);
  ...
  assert.equal(star('mr-perfect').ability.trigger.drawMaxUses,2);

  assert.equal(star('becky-lynch').hp,65);
  assert.equal(card('becky-lynch-dis-arm-her').submission.pressure,6);
  ...
  assert.deepEqual(card('john-cena-hustle-loyalty-respect').effect,{type:'hustleLoyaltyRespect',hpPct:0.5,adrenaline:2,draw:1});
...
test('v0.14.20 Perfect Execution keeps its once-per-Control discount and only draws on the first two successful Counters each match',()=>{
  const g=engineFor('mr-perfect'); const p=g.state().players.p1;
  p.deck.push(card('body-slam'),card('dropkick'),card('bulldog'),card('vertical-suplex'));
  let before=p.hand.length;
  assert.equal(g._ability('p1','counter',{incoming:card('body-slam'),counter:card('punch')}),true);
  assert.equal(p.hand.length,before+1); assert.equal(p.events.perfectExecutionPending,2);
  assert.equal(g._ability('p1','counter',{incoming:card('body-slam'),counter:card('punch')}),false);
  delete p.events.perfectExecutionPending; g._resetControlSequenceState('p1'); before=p.hand.length;
  assert.equal(g._ability('p1','counter',{incoming:card('body-slam'),counter:card('punch')}),true);
  assert.equal(p.hand.length,before+1);
  delete p.events.perfectExecutionPending; g._resetControlSequenceState('p1'); before=p.hand.length;
  assert.equal(g._ability('p1','counter',{incoming:card('body-slam'),counter:card('punch')}),true);
  assert.equal(p.hand.length,before); assert.equal(p.events.perfectExecutionDrawUses,2);
});
...
test('v0.14.20 Hustle, Loyalty, Respect is persistent and fires exactly once when Cena reaches 50% HP or less',()=>{
  ...
  assert.equal(p.hand.length,beforeTrigger+1);
