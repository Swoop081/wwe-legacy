...
import { BUILD_VERSION } from "../js/config/build.js";
...
test("v1.1.95 runtime build identity matches build.json",()=>{
  ...
  assert.equal(BUILD_VERSION,"1.1.95");
});
...
test("v1.1.95 cache-busting imports are stamped to the release version",()=>{
  const app=fs.readFileSync(new URL("../js/ui/app.js",import.meta.url),"utf8");
  assert.ok(app.includes('config/build.js?v=1.1.95'));
  assert.equal(app.includes('?v=1.1.86'),false);
});
